...
namespace FormCore
{
    public partial class Form1 : Form
    {
        ...
        private void DisplayPixelData(Bitmap bitmap, Myrect[] Geometry, int drawSize, int bitmapWidth, int bitmapHeight, out double drawTime, out double showTime, float dpi = 96, bool IsThreadCall = false)
        {
            var startTime = DateTime.Now;
            var device = CanvasDevice.GetSharedDevice();
            drawTime = 0;
            showTime = 0;
            using (var offscreen = new CanvasRenderTarget(device, bitmapWidth, bitmapHeight, dpi))         /*使用离离对象在屏幕外绘制图形*/
            {
                ...
                if (IsThreadCall)
                {
                    ...
                    try
                    {
                        BitmapData bmpData = bitmap.LockBits(new Rectangle(0, 0, bitmapWidth, bitmapHeight),                           /*将像素数组复制到位图对象中*/
